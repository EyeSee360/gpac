...
GF_Err gf_isom_add_sample_from_source(GF_ISOFile *movie, u32 trackNumber, u32 StreamDescriptionIndex, const GF_ISOSample *sample, GF_BitStream *src, u64 srcFileOffset)
{
	...
	if (trak->Media->handler->handlerType == GF_ISOM_MEDIA_OD) {
		...
		e = Media_AddSample(trak->Media, data_offset, sample, descIndex, 0);
		if (!e && sample->dataLength) e = gf_isom_datamap_add_file_data(trak->Media->information->dataHandler, src, srcFileOffset, sample->dataLength);
	}
	if (e) return e;
...
GF_Err gf_isom_add_sample_group_info(GF_ISOFile *movie, u32 track, u32 grouping_type, void *data, u32 data_size, Bool is_default, u32 *sampleGroupDescriptionIndex)
{
	...
	if (!entry) return GF_OUT_OF_MEM;
	entry->data = gf_malloc(sizeof(char) * data_size);
	if (!entry->data) {
		gf_free(entry);
		return GF_OUT_OF_MEM;
	}
